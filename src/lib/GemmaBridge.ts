...
interface SimplifiedNativeBridge {
  ...
  generateResponseWithImages(prompt: string, imagePaths: string[]): Promise<{
    response: string;
    inferenceTimeMs: number;
    tokenCount: number;
  }>;
...
export interface EnhancedGemmaBridge {
  ...
  generateResponseWithImages(prompt: string, imagePaths: string[]): Promise<string>;
...
class SimplifiedGemmaBridgeImpl implements EnhancedGemmaBridge {
  ...
  async generateResponseWithImages(prompt: string, imagePaths: string[]): Promise<string> {
    const validation = this.validateInput(prompt);
    if (!validation.isValid) {
      throw new Error(validation.error);
    }

    const result = await this.nativeBridge.generateResponseWithImages(prompt, imagePaths);
    return result.response;
  }
...
// Auto-initialize
SimplifiedGemmaBridge.initialize().catch();
