...
export class ModelManager {
  ...
  async loadModel(
    ...
    try {
      ...
      // Step 2: Check if model is already extracted to internal storage
      let modelPath = await this.fileManager.getModelPath(modelFileName);

      if (!modelPath) {
        
        this.updateStatus({ loadProgress: 0.2 });
        try {
          modelPath = await this.fileManager.extractBundledModel(
            modelFileName
          );
      ...
      this.updateStatus({ loadProgress: 0.5, modelPath });
      ...
      this.updateStatus({ loadProgress: 0.7 });

      // Step 4: Load model with retry logic
      ...
      if (success) {
        ...
        throw new Error("Model loading failed after all retry attempts");
      }
    } catch (error) {
      const errorMessage =
